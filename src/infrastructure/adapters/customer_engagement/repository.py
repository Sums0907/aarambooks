...
class CustomerEngagementRepository:
    ...
    async def create_engagement(self, record: CustomerEngagementRecord) -> CustomerEngagementRecord:
        ...
        try:
            ...
            if existing:
                # engagement_id is a stable hash of the caller's own retry key (e.g. NDR
                # Queue Poller derives it from queue_item_id - see ndr_queue_poller.py), so a
                # colliding engagement_id IS a retry of the same underlying work item by
                # design, not an anomaly. action_request_id, in contrast, is freshly
                # regenerated by the orchestrator on every single run (see
                # orchestrator.py's `f"act_{uuid.uuid4().hex[:8]}"`), so it will *never*
                # match across retries - comparing it here used to make every retry of an
                # already-engaged queue item fail permanently with a misleading error,
                # burning the item's entire max_retries budget on this check alone rather
                # than on whatever actually went wrong the first time. Always treat a
                # colliding engagement_id as the same logical engagement and return it.
