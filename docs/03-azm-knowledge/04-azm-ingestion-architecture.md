...
6. **AZM-Derived Knowledge Construction (Ingestion Time Only):** During ingestion, AZM performs limited, governed knowledge derivation by recognizing relationships across BS contracts. **This derivation happens at ingestion time — not at Brain query time. AZM is not a runtime reasoning engine.** These relationships are persisted as AZM-Derived knowledge nodes with derivation provenance that explicitly records which source contracts and which derivation method produced the relationship. **AZM-Derived knowledge must be clearly distinguished from Source-Declared knowledge in the persistent store.**
