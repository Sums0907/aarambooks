...
## 6. Context Reconciliation & Source of Truth (ADR-005)

ShopDeck is formally defined as the primary Operational Context Provider for the Aaram Brain. It is the canonical aggregator of customer identity, order lifecycle, payment, shipment tracking, and return/RTO state.

### Source of Truth Boundaries

- **ShopDeck (Primary Context):** Tracks the end-to-end customer journey, order state (`seller_last_status`), shipment state (`awb_no`, `delivery_time`, `rto_initiated`), and payment state.
- **AaramInventory (Quantity Authority):** Remains the sole authority for physical inventory. The Brain queries AaramInventory solely for `quantity_on_hand` (using `sku_id` from ShopDeck). The `confidence_score` is explicitly ignored by the Brain.
- **AaramPacking (Action Target Only):** AaramPacking is purely a warehouse execution system. It is **NOT** a Context Provider. Its outputs are synchronized back to ShopDeck, meaning ShopDeck already contains the authoritative fulfillment state. 

### Context Mapping

| ShopDeck Field | Brain Context Field | Notes |
|---|---|---|
| `order_line_items.seller_last_status` | `FulfillmentContext.fulfillment_status` | Authoritative |
| `order_line_items.awb_no` | `ShipmentContext.tracking_number` | Authoritative |
| `order_line_items.customer_last_status` | `ShipmentContext.status` | Authoritative |
| `order_summary.order_id` | `OrderContext.order_id` | Authoritative |
| `customer_info.customer_number` | `CustomerContext.phone` | Decryption required |
| `order_line_items.sku_id` | `InventoryContext.sku_id` | Used to query AaramInventory |

### Strategic Decisions
- Brain Core does not own operational truth; it models semantic requirements based on ShopDeck's structured payload.
- M2M Authentication issues for ShopDeck headless connectivity are acknowledged, but do not invalidate this architecture.
